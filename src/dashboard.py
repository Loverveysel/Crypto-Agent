    ...
    async def panic_close_all():
        ...
        for symbol in open_symbols:
            try:
                ...
                unsubscribe_msg = {"method": "UNSUBSCRIBE", "params": [f"{symbol.lower()}@kline_1m"], "id": int(time.time())}
    ...
    with ui.tab_panels(tabs, value=dash_tab).classes('w-full bg-transparent p-0'):
        ...
        with ui.tab_panel(report_tab).classes('p-4'):
            with ui.row().classes('items-center justify-between w-full mb-4'):
                ui.label('📊 DETAYLI STRATEJİ VE PERFORMANS RAPORU').classes('text-lg font-bold text-white')
                ...
                ui.button('RAPORU YENİLE', icon='refresh', on_click=refresh_report).props('outline size=sm')

            # Tablo Yapısı
            columns = [
                {'name': 'time', 'label': 'Zaman', 'field': 'time', 'sortable': True, 'align': 'left'},
                {'name': 'symbol', 'label': 'Coin', 'field': 'symbol', 'sortable': True, 'align': 'left'},
                {'name': 'action', 'label': 'AI Kararı', 'field': 'action', 'align': 'center'},
                {'name': 'confidence', 'label': 'Güven', 'field': 'confidence', 'sortable': True, 'align': 'center'},
                {'name': 'ai_reason', 'label': 'Giriş Mantığı', 'field': 'ai_reason', 'align': 'left', 'classes': 'ellipsis', 'style': 'max-width: 300px; overflow: hidden; text-overflow: ellipsis;'},
                {'name': 'trade_result', 'label': 'Sonuç (PnL)', 'field': 'pnl', 'sortable': True, 'align': 'right'},
            ...
            report_table.add_slot('body-cell-trade_result', '''
                <q-td :props="props">
        ...
        with ui.tab_panel(history_tab).classes('p-4'):
            ui.label('📜 KAPANMIŞ İŞLEMLER (ÖZET)').classes('text-lg font-bold mb-4 text-white')
